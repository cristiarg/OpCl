...
bool sendQuit(CProtocol& prot)
{
  ...
  //
  std::string mesQuit;
  st = prot.recvMessage(mesQuit);
  CMessageDecoder decRespQuit{ mesQuit };
  assert(decRespQuit.isValid() && decRespQuit.getType() == EMessageType::eOK);
  return true;
}
...
std::optional<bool> executeWait(CProtocol& prot)
{
...
void scen01Loop(CProtocol& prot, const bool steppedMode)
{
  ...
  while (res) {
    ...
    if (steppedMode) {
      const auto resExecWait = executeWait(prot);
      res = res && resExecWait.value_or(res);
    }
  }
}
